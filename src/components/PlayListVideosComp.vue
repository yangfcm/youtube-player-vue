...
<template>
  ...
  <app-video-item v-for="item in playListItems" :key="(item.id as string)" :video="{
  ...
  }" :playListId="playListId" class="mb-3"></app-video-item>
  <app-more-button v-if="hasMore" :loading="status === AsyncStatus.LOADING" @onLoadMore="fetchMore">More
    Videos</app-more-button>
</template>